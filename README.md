...
* `build-was-seed-druid-tree`: reads the seed object source xml file from `/web-archiving-stage/seed` directory and creates the druid tree under `/dor/workspace`. The content folder contains the `source.xml` that has been generated by was-registrar.
